...
func (repo orderRepository) GetOrderById(ctx context.Context, id int64) *entity.Order {
	_, span := repo.tracer.Start(ctx, "GetOrderById Repository")
	defer span.End()

	span.AddEvent("Getting Data from DB...")
	...
	span.AddEvent("Got Data from DB...")
...
func NewOrderRepository(db *gorm.DB) OrderRepository {
	return &orderRepository{db: db, tracer: otel.Tracer("OrderApi Repository")}
}
